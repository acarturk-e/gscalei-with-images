...
class XDsxDataset(Dataset):
    def __init__(self, xs: Tensor, dsx_dir: str):
        self.xs = xs
        self.dsx_dir = dsx_dir

        logging.info(f"Created joint x--dsx loader.")
    ...
    def __getitem__(self, idx: int) -> tuple[Tensor, Tensor, Tensor]:
        xi = self.xs[idx]
        with open(os.path.join(self.dsx_dir, "dsxs_bw_hards", f"{idx}.pth"), "rb") as f:
            dsxi_bw_hards = torch.load(f, weights_only=True)
        with open(os.path.join(self.dsx_dir, "dsxs_hard_obs", f"{idx}.pth"), "rb") as f:
            dsxi_hard_obs = torch.load(f, weights_only=True)
        return xi, dsxi_bw_hards, dsxi_hard_obs


def get_zx_data(data_dir: str) -> tuple[Tensor, Tensor]:
...
def create_logger(log_dir: str) -> logging.Logger:
    ...
    h_file = logging.FileHandler(os.path.join(log_dir, "train_autoenc_cci.log"), mode="w")
...
def main(
    ...
    zs_obs, xs_obs = get_zx_data(args.data_dir)
    x_dsx_dataset = XDsxDataset(xs_obs, args.data_dir)
    ...
    [train_dataset, valid_dataset] = random_split(x_dsx_dataset, [0.9, 0.1])
    ...
    # Create model
    autoenc = Autoencoder(n).to(local_rank)

    if args.load_checkpoint:
        autoenc.load_state_dict(torch.load(os.path.join(args.data_dir, "autoenc.pth"), weights_only=True))

    encoder = DDP(autoenc.get_submodule("encoder"), device_ids=[local_rank])
    decoder = DDP(autoenc.get_submodule("decoder"), device_ids=[local_rank])
    ...
    def loss_fn(xb: Tensor, dsxb: Tensor, compute_main_loss: bool) -> Tensor:
        ...
        if compute_main_loss:
            ...
            jb = torch.zeros(xb.shape + (n,), device=local_rank)
            ...
            loss_main = (dt - torch.eye(n, device=local_rank)).abs().sum()
        else:
            loss_main = torch.zeros((), device=local_rank)
