...
def create_logger(log_dir: str) -> logging.Logger:
    ...
    h_file = logging.FileHandler(os.path.join(log_dir, "train_ldr_cci.log"), mode="w")
...
def train_ldr(
    ...
    # Create model
    ldr_model = DDP(LdrCnn().to(local_rank), device_ids=[local_rank])
...
def main(
    ...
    if rank != 0 or local_rank != 0: return
    ...
    # NOTE: We do this in batches to not get OOM errors
    logger.info("Starting score difference computation.")

    os.makedirs(os.path.join(args.data_dir, "dsxs_bw_hards"), exist_ok=True)
    os.makedirs(os.path.join(args.data_dir, "dsxs_hard_obs"), exist_ok=True)

    for x_idx, xi in enumerate(xs_obs):
        xi = xi.to(local_rank)

        # 1. Hard intervention pairs
        dsxi_bw_hards = torch.zeros((n,) + xi.shape, device=local_rank)
        for env_idx in range(n):
            ldr_model = ldr_bw_hards[env_idx]
            logging.info(ldr_model(xi.unsqueeze(0)).squeeze(0))
            with torch.no_grad():
                dsxi_bw_hards[env_idx] = score_diff_from_ldr(ldr_model.forward, xi.unsqueeze(0)).squeeze(0)
        logging.info(dsxi_bw_hards)
        logging.info(f"Opening file {os.path.join(args.data_dir, 'dsxs_bw_hards', f'{x_idx}.pth')}")
        with open(os.path.join(args.data_dir, "dsxs_bw_hards", f"{x_idx}.pth"), "rb") as f:
            logging.info("Opened file, before torch.save call")
            torch.save(dsxi_bw_hards, f)
        logging.info("saved bw hards")

        # 2. Hard interventions (one set suffices) and observational domain
        dsxi_hard_obs = torch.zeros((n,) + xi.shape, device=local_rank)
        for env_idx in range(n):
            ldr_model = ldr_hard_obs[env_idx]
            with torch.no_grad():
                dsxi_hard_obs[env_idx] = score_diff_from_ldr(ldr_model.forward, xi.unsqueeze(0)).squeeze(0)
        logging.info(f"before save hard obs to {os.path.join(args.data_dir, 'dsxs_hard_obs', f'{x_idx}.pth')}")
        torch.save(dsxi_hard_obs, os.path.join(args.data_dir, "dsxs_hard_obs", f"{x_idx}.pth"))
        # torch.save(dsxi_hard_obs.detach().cpu(), os.path.join(args.data_dir, "dsxs_hard_obs", f"{x_idx}.pth"))
        logging.info("saved hard obs")
